...
export default function Dashboard({ guildId, guildName, guildIcon, memberCount, onBack, user, onLogout }) {
  ...
  const formatWelcomeText = (rawText) => {
    ...
    return parts.map((part, index) => {
      ...
      const chMentionMatch = part.match(/^<#(\d+)>$/);
      if (chMentionMatch) {
        const targetId = chMentionMatch[1];
        const targetCh = channels.find(c => c.id === targetId);
        const chName = targetCh ? targetCh.name : 'channel';
        return (
          <span key={`mention-id-${targetId}-${index}`} className="discord-mention-channel">
            #{chName}
          </span>
        );
      }
                    ...
                    {settings.welcome.enabled && (
                      <div className="welcome-split-layout" style={{ borderTop: '1px solid var(--border-color)', paddingTop: '16px' }}>
                        <div className="welcome-settings-column">
                          ...
                          <div>
                            <label style={{ display: 'block', fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '6px' }}>Welcome Message Template (Type # to mention any channel)</label>
                            <ChannelMentionInput
                              isTextArea={true}
                              ...
                              channels={channels}
                              className="glass-input"
                              placeholder="Welcome {user} to the server! Type # to mention any channel..."
                              rows={3}
                          ...
                          <div className="glass-panel" style={{
                            ...
                            <div>
                              ...
                              <ChannelMentionInput
                                ...
                                className="glass-input"
                                rows={3}
                                placeholder="**BUY ALL THINGS FROM HERE <#153338781401100410>** (Type # to mention channel)"
                                  ...
                                  {settings.welcome.embedFields.map((field, idx) => (
                                    <div key={idx} style={{ padding: '10px', borderRadius: '8px', backgroundColor: 'rgba(0,0,0,0.2)', border: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                      ...
                                      <ChannelMentionInput
                                        ...
                                        className="glass-input"
                                        rows={2}
                                        placeholder="Field Value (Type # to mention channel)"
                            ...
                            <div>
                              ...
                              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                ...
                                <ChannelMentionInput
                                  ...
                                  channels={channels}
                                  className="glass-input"
                                  placeholder="@everyone Welcome to my Server (Type # to mention channel)"
                                  style={{ flex: 1 }}
                                />
