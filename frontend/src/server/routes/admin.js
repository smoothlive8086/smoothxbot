...
// GET /guilds/:guildId/members/:memberId - Get details of a single member
router.get('/guilds/:guildId/members/:memberId', async (req, res) => {
  ...
  try {
    ...
    const member = await guild.members.fetch(memberId).catch((err) => {
      console.warn(`[Server Admin] Failed to fetch member ${memberId} in guild ${guildId}:`, err.message);
      return null;
    });
    if (!member) {
      console.log(`[Server Admin] Member ${memberId} not found in guild. Fetching user globally...`);
      const user = await client.users.fetch(memberId).catch((err) => {
        console.warn(`[Server Admin] Failed to fetch user ${memberId} globally:`, err.message);
        return null;
      });
      if (!user) {
        console.warn(`[Server Admin] User ${memberId} could not be resolved globally.`);
    ...
    console.error(`[Server Admin Error] Failed to get member details for ${memberId}:`, error.message);
...
// GET /admin/bot-settings - Get current bot settings and client stats
router.get('/bot-settings', async (req, res) => {
  try {
    ...
    // Add current bot client user details if available
    ...
    console.error('[Admin API] Failed to fetch bot settings:', error.message);
    res.status(500).json({ error: 'Failed to fetch bot settings: ' + error.message });
  }
});

// POST /admin/bot-settings - Update bot settings and presence in real-time
router.post('/bot-settings', async (req, res) => {
  ...
  if (!allowedStatuses.includes(status)) {
    return res.status(400).json({ error: 'Invalid status value. Must be online, idle, dnd, or invisible.' });
  }

  const actType = activityType !== undefined ? parseInt(activityType) : 4;
  const allowedActivityTypes = [0, 1, 2, 3, 4, 5]; // Playing, Streaming, Listening, Watching, Custom, Competing
  if (!allowedActivityTypes.includes(actType)) {
  ...
  try {
    ...
    // Apply real-time presence change to Discord client
    if (client && client.user) {
      const activity = {
        name: actType === 4 ? 'Custom Status' : (activityText || ''),
        type: actType
      };
      if (actType === 4) {
      ...
      console.log(`[Bot] Real-time status updated from admin panel: ${status} | type: ${actType} | text: ${activityText}`);
    } else {
      console.warn('[Bot] Discord client is not ready. Saved status will be applied when bot logs in.');
    }
    ...
    console.error('[Admin API] Failed to update bot settings:', error.message);
