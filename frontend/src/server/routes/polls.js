...
const { renderPollEmbed, renderPollComponents, sendPollMessage, updatePollMessage } = require('../../bot/utils/pollHelper');
...
router.post('/:guildId', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // Send to Discord
    const message = await sendPollMessage(channel, poll, guild);
    ...
    if (guild) {
      ...
      if (channel) {
        try {
          ...
          if (message) {
            await updatePollMessage(message, poll, guild);
          }
        } catch (msgErr) {
