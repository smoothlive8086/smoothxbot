...
router.post('/admin-login', async (req, res) => {
  const { username, password, hwid: clientHwid } = req.body;
  if (!username || !password) {
    return res.status(400).json({ error: 'Username and password are required' });
  }

  try {
    const name = config.keyauthName || "Smoothlive8086's Application";
    const ownerid = config.keyauthOwnerId || "kxhJUGG37M";
    const version = config.keyauthVersion || "1.0";
    const secret = config.keyauthSecret || "6132d16a79c00aedb747efb145ffcbf2f729adbb4dbb0c33088cce80ee89a3e0";
    ...
    // 2. Validate login credentials (username and password) with KeyAuth API 1.2 using client device HWID
    const hwid = clientHwid || crypto.createHash('sha256').update((req.ip || '') + (req.headers['user-agent'] || '')).digest('hex');
    const loginUrl = `https://keyauth.win/api/1.2/?type=login&username=${encodeURIComponent(username)}&pass=${encodeURIComponent(password)}&sessionid=${sessionid}&name=${encodeURIComponent(name)}&ownerid=${ownerid}&secret=${secret}&hwid=${encodeURIComponent(hwid)}`;
