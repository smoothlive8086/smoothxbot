...
const { getModerationLogs, clearModerationLogs } = require('../../database/logManager');
...
router.post('/:guildId/tickets-embed', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      .setTitle(settings.tickets.title || 'Support Ticket System')
      .setDescription(settings.tickets.welcomeMessage || 'Click a button below to open a ticket. Our support team will help you shortly.')
      .setColor('#2563eb')
      .setThumbnail(guild.iconURL({ size: 128 }))
    ...
    options.forEach((opt, index) => {
      ...
      else if (opt.style === 'success') btnStyle = ButtonStyle.Success;
      else if (opt.style === 'danger') btnStyle = ButtonStyle.Danger;

      const button = new ButtonBuilder()
        .setCustomId(`open_ticket_${index}`)
        .setLabel(opt.label || `Ticket #${index + 1}`)
        .setStyle(btnStyle);
      ...
      if (opt.emoji && opt.emoji.trim()) {
        try {
          ...
          console.warn(`[Ticket Panel] Could not set emoji "${opt.emoji}":`, e.message);
        }
      }
...
// Clear moderation logs for a guild
router.delete('/:guildId/logs', authMiddleware, async (req, res) => {
  const { guildId } = req.params;
  try {
    await clearModerationLogs(guildId);
    res.json({ success: true, message: 'Logs cleared successfully' });
  } catch (error) {
    console.error(`[Server Settings Error] Failed to clear moderation logs for ${guildId}:`, error.message);
    res.status(500).json({ error: 'Failed to clear moderation logs' });
  }
});
