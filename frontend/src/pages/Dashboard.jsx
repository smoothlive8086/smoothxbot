...
export default function Dashboard({ guildId, guildName, guildIcon, memberCount, onBack, user, onLogout }) {
  ...
  const unwrapDiscordProxyUrl = (urlStr) => {
    if (!urlStr || typeof urlStr !== 'string') return '';
    let url = urlStr.trim();
    if (/images-ext-\d+\.discordapp\.net\/external\//i.test(url)) {
      const encodedMatch = url.match(/\/external\/[^\/]+(?:\/%3Fformat%3D[^\/]+)?\/(https?%3A%2F%2F.+)$/i);
      if (encodedMatch) {
        try {
          url = decodeURIComponent(encodedMatch[1]);
        } catch (e) {}
      } else {
        const protoMatch = url.match(/\/external\/[^\/]+(?:\/%3Fformat%3D[^\/]+)?\/(https?)\/(.+)$/i);
        if (protoMatch) {
          url = `${protoMatch[1]}://${protoMatch[2]}`;
        }
      }
      if (!url.includes('cdn.discordapp.com/attachments/')) {
        try {
          ...
          url = url.replace(/([?&])(?:width|height|format)=\w+/gi, '').replace(/[?&]+$/, '').replace(/\?&/, '?');
        }
      }
    }
    return url;
                        ...
                        <div className="welcome-settings-column" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                          ...
                          <div style={{
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                const val = unwrapDiscordProxyUrl(e.target.value);
                            ...
                            />
                            <p style={{ fontSize: '0.76rem', color: '#64748b', margin: '6px 0 0 0' }}>
                              Supports direct image links &amp; animated GIFs (Tenor, Giphy, Discord media). Discord proxy URLs are automatically converted.
                            </p>
