...
/**
 * Clears all moderation logs for a guild.
 * @param {string} guildId 
 */
async function clearModerationLogs(guildId) {
  memoryLogs.set(guildId, []);
  if (mongoose.connection.readyState === 1) {
    try {
      await ModerationLog.deleteMany({ guildId });
    } catch (e) {
      console.warn(`[LogManager] MongoDB log delete failed: ${e.message}`);
    }
  }
  return true;
}

module.exports = { logModerationAction, getModerationLogs, clearModerationLogs };
