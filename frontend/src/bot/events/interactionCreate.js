    ...
    if (interaction.isButton()) {
      ...
      if (interaction.customId === 'open_ticket' || interaction.customId.startsWith('open_ticket_')) {
        try {
          ...
          if (interaction.customId.startsWith('open_ticket_')) {
            const parsedIdx = parseInt(interaction.customId.replace('open_ticket_', ''), 10);
            if (!isNaN(parsedIdx)) optionIndex = parsedIdx;
          }

          const options = (Array.isArray(settings.tickets.options) && settings.tickets.options.length > 0)
          ...
          const labelSlug = (selectedOption.label || 'ticket').toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-').substring(0, 15);
          const ticketChannelName = `${labelSlug}-${member.user.username.toLowerCase()}`.substring(0, 100);

          const parentId = selectedOption.categoryId || settings.tickets.categoryId || null;
          const supportRoleId = selectedOption.supportRoleId || settings.tickets.supportRoleId || null;
          ...
          const welcomeTemplate = selectedOption.ticketMessage || settings.tickets.ticketMessage || 'Welcome {user}! Please describe your issue. Support staff will assist you shortly.';
          ...
          const embed = new EmbedBuilder()
            .setTitle(selectedOption.title || settings.tickets.title || 'Support Ticket')
      ...
      if (interaction.customId.startsWith('pv_')) {
        try {
          ...
          const { renderPollEmbed, renderPollComponents, updatePollMessage } = require('../utils/pollHelper');
          ...
          if (poll.settings.expiresAt && new Date(poll.settings.expiresAt) <= new Date()) {
            ...
            await updatePollMessage(interaction.message, poll, interaction.guild);
