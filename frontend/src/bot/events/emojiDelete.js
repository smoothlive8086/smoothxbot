  ...
  async execute(emoji) {
    ...
    try {
      ...
      await dispatchAuditLog(emoji.guild.id, {
        ...
        details: `Emoji :${emoji.name}: deleted`,
        timestamp: new Date()
      });

      const config = await getGuildSettings(emoji.guild.id);
      if (config?.antinuke?.enabled && config.antinuke.antiEmoji && executor) {
        await handleAntiAction(emoji.guild, executor, 'emoji_delete', config.antinuke);
      }
    } catch (err) {
      console.error(`[Event Error] emojiDelete failed:`, err.message);
    }
  }
};
